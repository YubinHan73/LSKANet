...
class Dev_AttentionModule(BaseModule):
    def __init__(self, dim):
        ...
        self.conv4_1 = nn.Conv2d(dim, dim, (1, 21), padding=(0, 10), groups=dim)
        self.conv4_2 = nn.Conv2d(dim, dim, (21, 1), padding=(10, 0), groups=dim)

        self.conv5_1 = nn.Conv2d(dim, dim, (1, 31), padding=(0, 15), groups=dim)
        self.conv5_2 = nn.Conv2d(dim, dim, (31, 1), padding=(15, 0), groups=dim)
    ...
    def forward(self, x):
        ...
        attn_0_1 = self.conv0_1(attn)
        attn_0_2 = self.conv0_2(attn)
        attn_0 = attn_0_1 + attn_0_2
        ...
        attn_4_1 = self.conv4_1(attn)
        attn_4_2 = self.conv4_2(attn)
        attn_4 = attn_4_1 + attn_4_2

        attn_5_1 = self.conv5_1(attn)
        attn_5_2 = self.conv5_2(attn)
        attn_5 = attn_5_1 + attn_5_2

        # attn = attn + attn_0 + attn_1 + attn_2
        # attn = attn + attn_0 + attn_1 + attn_2 + attn_3 + attn_4 +attn_5
...
class Ana_AttentionModule(BaseModule):
    def __init__(self, dim):
        super().__init__()
...
class SpatialAttention(BaseModule):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        # self.norm_1 = build_norm_layer(dict(type='BN', requires_grad=True), dim)[1]
        self.proj_1 = nn.Conv2d(dim, dim, 1)
        # self.activation = nn.GELU()
        self.activation = nn.ReLU()
        self.dev_spatial_gating_unit = Dev_AttentionModule(dim)
        self.ana_spatial_gating_unit = Ana_AttentionModule(dim)
        self.proj_2 = nn.Conv2d(dim, dim, 1)
        # self.norm_2 = build_norm_layer(dict(type='BN', requires_grad=True), dim)[1]
    ...
    def forward(self, x):
        ...
        x_dev = self.dev_spatial_gating_unit(x)
        x_dev = self.proj_2(x_dev)

        x_ana = self.ana_spatial_gating_unit(x)
        x_ana = self.proj_2(x_ana)

        # x = shorcut + x_dev + x_ana
        x = shorcut + x_dev
        return x
...
class HRMscanv7dev(BaseModule):
    """hrmscanv7，只做ana_attn"""
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(HRMscanv7dev, self).__init__(init_cfg)
        ...
        for i in range(self.num_stages):
            fusion_mscan_block = nn.ModuleList([Block(dim=embed_dims[i], mlp_ratio=mlp_ratios[i], drop=drop_rate,
                                                      drop_path=dpr[cur + j], norm_cfg=norm_cfg)
                                                for j in range(depths[i])])
            batch_norm = build_norm_layer(norm_cfg, embed_dims[i])[1]
            cur += depths[i]
            setattr(self, f"fusion_mscan_block{i + 1}", fusion_mscan_block)
    ...
    def forward(self, x):
        ...
        for i in range(len(y_list)):
            x = y_list[i]
            fusion_mscan_block = getattr(self, f"fusion_mscan_block{i + 1}")
            batch_norm = getattr(self, f"batch_norm{i + 1}")
            for blk in fusion_mscan_block:
                x = blk(x)
            x = batch_norm(x)
            outs.append(x)
    ...
    def train(self, mode=True):
        ...
        layer freezed."""
        super(HRMscanv7dev, self).train(mode)
        self._freeze_stages()
