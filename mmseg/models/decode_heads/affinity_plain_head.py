...
class AffinityFU_Plain_Head(BaseDecodeHead):
    ...
    def __init__(self, interpolate_mode='bilinear', **kwargs):
        ...
        for i in range(num_inputs):
            self.off_blocks.append(
                ConvModule(self.in_channels[0], self.in_channels[i], kernel_size=1, stride=1, padding=0,
                           conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg, act_cfg=None))
    ...
    def forward(self, inputs):
        ...
        for i in range(len(x)):
            if i == 0:
                ...
                out_feat = self.off_blocks[i](in_feat)
                x[i] = out_feat
